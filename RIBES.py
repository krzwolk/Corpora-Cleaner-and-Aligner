...
def kendall(ref, hyp):
    ...
    if sys.version_info.minor > 1:
        mapped_ref = _ref.decode('hex').decode(encoding="utf_16_be")
    else:
    ...
    # decode Unicode (UTF-16 BigEndian) sequences to UTF-8
    if sys.version_info.minor > 1:
        mapped_hyp = _hyp.decode('hex').decode(encoding="utf_16_be")
    else:
        ...
        for i in range (len(hyp)):
            ...
            for r in range(len(REFS)):
                try:
                    ...
                    print ("Error in " + REFS[r].filename + " line " + str(i))
                    raise e
            ...
            if self.__sent and self.__output != None:
                print ("%.6f alpha=%f beta=%f %s sentence %d" % (_best_ribes, self.__alpha, self.__beta, hyp.filename, i))
...
def outputRIBES (options, args, file=sys.stdout):
    # print start time
    print ("# RIBES evaluation start at " + str(datetime.datetime.today()))
    ...
    for _ref in options.ref:
        if debug > 0:
            ...
            print ("# reference file [" + str(len(REFS)) + "] : " + _ref)
    ...
    for i in range(len(args)):
        if debug > 0:
            ...
            print ("# system output file [" + str(i) + "] : " + args[i])
        ...
        # print resutls
        print ("%.6f alpha=%f beta=%f %s" % (best_ribes, options.alpha, options.beta, args[i]))

    # print end time
    print ("# RIBES evaluation done at " + str(datetime.datetime.today()))
